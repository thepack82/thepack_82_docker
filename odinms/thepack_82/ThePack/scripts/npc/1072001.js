...
function action(mode, type, selection) {
    if (mode == -1)
        ...
        if (mode == 1)
            ...
            status--;
        if (status == 0 && cm.getQuestStatus(100007) == net.sf.odinms.client.MapleQuestStatus.Status.STARTED)
            status = 3;
        if (status == 0) {
            if (cm.getQuestStatus(100007) == net.sf.odinms.client.MapleQuestStatus.Status.COMPLETED) {
                cm.sendOk("You're truly a hero!");
                cm.dispose();
            } else if (cm.getQuestStatus(100006).getId() >= net.sf.odinms.client.MapleQuestStatus.Status.STARTED.getId()) {
                cm.completeQuest(100006);
                if (cm.getQuestStatus(100006) == net.sf.odinms.client.MapleQuestStatus.Status.COMPLETED)
